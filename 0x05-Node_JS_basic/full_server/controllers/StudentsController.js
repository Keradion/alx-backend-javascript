...
class StudentsController {
  static async getAllStudents (request, response) {
    ...
    try {
      ...
      for (const filed of fileds) {
        result = result + `Number of students in ${filed}:  ${data[filed].length}. List: ${data[filed].join(',  ')}<br>`;
      }
      response.status(200);
  ...
  static async getAllStudentsByMajor (request, response) {
    const major = request.params.major.toUpperCase(); // get the requested major name
    let result = '';
